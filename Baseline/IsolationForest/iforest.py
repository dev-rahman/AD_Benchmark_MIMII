"""
Model 2 — Isolation Forest with rich MFCC features.
...
Usage   : python Baseline/IsolationForest/iforest.py [--machine fan|pump|valve] [--all]
"""
...
def run(machine: str) -> dict:
    ...
    scores = -model.score_samples(X_test)   # negate: higher = more anomalous

    result = evaluate(y_true, scores, machine=machine, method="IsolationForest")
    save_results([result], os.path.join(RESULTS_DIR, f"iforest_{machine}.csv"))
    return result


def main():
    parser = argparse.ArgumentParser(description="Isolation Forest baseline")
    ...
    if args.all:
        save_results(results, os.path.join(RESULTS_DIR, "iforest_all.csv"))
